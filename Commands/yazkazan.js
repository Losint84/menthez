...
var kelimeler = ['losint', 'menthe', 'development', 'hamburger', 'pizza', 'turta', 'çörek', 'elma', 'armut', 'kalpler', 'puffy', 'balık', 'balina', 'levrek', 'helva', 'kıyafet', 'tayt', 'şort'];
      ...
      message.channel.send('Kelime hazırlanıyor, bekleyin!').then(sent2 => {
        setTimeout(() => {
          ...
          message.channel.send(`${member} ${message.author}, kelimeniz: \`${kelime}\``);
            ...
            return message.channel.send(`${answer.first().author} doğru cevabı verdi!\n Tebrikler !`).then(m => m.delete({ timeout: 20000 }))
