...
exports.run = async (client, message, args) => {
    ...
    message.channel.send(new Discord.MessageEmbed().setTitle('Onaylıyor musunuz?').setColor('RANDOM').setDescription(`**${emojis.size}** adet emoji indirilip, \`ZIP\` dosyası olarak verilecek.`)).then(m => {
        m.react('✅');
        m.react('❌');
        const emojies = ['✅', '❌'];
                ...
                case '✅':
                    ...
                    emojis.forEach(async emoji => {
                        const timeout = setTimeout(async () => {
                            ...
                            await m.edit(new Discord.MessageEmbed().setTitle('Menthe Emoji İndirme').setColor('RANDOM').addField('Toplam Emoji Sayısı', emojis.size, true).addField('Kaç Emoji Kaldı', kalan).addField('Yüzdelik', `Yüklenen: %${yuzdeHesapla(kalan, emojis.size)}`).setFooter(`Yaklaşık ${yaklasikSure(kalan)} süre kaldı.`))
                            await clearTimeout(timeout)
                            kalan = kalan - 1;
                            if (kalan == 0) {
                                const file = new AdmZip();
                                file.addLocalFolder('./emojiler', `Menthe Emoji (${emojiAd})`);
                                fs.writeFileSync('/mentheemoji.zip', file.toBuffer());
                                const embed = new Discord.MessageEmbed().setTitle('Menthe Emoji İndirme').setColor('RANDOM').setDescription(`**İşlem tamamlandı!** Toplam \`${emojis.size}\` adet emoji indirildi.`)
                                m.edit(embed);
                                message.channel.send({
                                    files: [{ name: 'mentheemoji.zip', attachment: '/mentheemoji.zip' }]
                                }).then(() => {
                                    fs.promises.rmdir('emojiler', { recursive: true }).then(() => console.log("Emojiler Klasörü Silindi ' Menthe ♥ Losint"))
                                });
                            }
                        }, yuklenen * 2000);
                        yuklenen = yuklenen + 1;
